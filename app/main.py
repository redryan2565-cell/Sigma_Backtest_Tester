        ...
        if view_mode == "Run Backtest":
            ...
            if 'previous_ticker_input' in st.session_state:
                if st.session_state['previous_ticker_input'] != ticker:
                    ...
                    if not preset_ticker_update_this_run:
                        ...
                        # If ticker is not one of the universal preset tickers, select "None"
                        if ticker_upper not in universal_preset_tickers:
                            ...
                            # Set universal_preset_selector to "None" BEFORE rerun (will be used in next run before widget creation)
                            ...
                            if preset_name in ['TQQQ', 'SOXL', 'QLD'] and get_universal_preset:
                                ...
                                if preset_name != current_loaded:
                                    ...
                                    if universal_preset:
                                        ...
                                        # Set universal_preset_selector BEFORE rerun (will be used in next run before widget creation)
            ...
            if PRESETS_AVAILABLE and get_preset_manager:
                ...
                if universal_preset_options:
                    # Sync universal_preset_selector with universal_preset_loaded BEFORE widget creation
                    # This ensures the radio button reflects the correct state without warnings
                    ...
                    if selected_universal == "None":
                        # Clear preset when "None" is selected
                        if 'universal_preset_loaded' in st.session_state:
                            del st.session_state['universal_preset_loaded']
                        if 'universal_preset' in st.session_state:
                            del st.session_state['universal_preset']
                        # No rerun needed - just visualization
                    elif selected_universal:
                        ...
                        if selected_universal != current_loaded and get_universal_preset:
                            ...
                            if universal_preset:
                                ...
                                # Trigger rerun to apply values
                                st.rerun()
                    # Note: else block removed - "None" is always an option, so selected_universal is always truthy
            ...
            if enable_tp or enable_sl:
                ...
                with st.expander("🔍 개념 검증", expanded=False):
                    st.markdown("""
                    **Hysteresis(히스테리시스)**는 트리거 후 바로 다시 작동하는 것을 막는 완충 구간입니다.

                    즉, TP(이익실현)나 SL(손절)이 한 번 작동한 뒤, 가격이 일정 범위 밖으로 다시 움직이기 전까지는 재작동하지 않게 하는 장치입니다.

                    → **목적**: 시장의 미세한 흔들림(노이즈)으로 인한 중복 트리거 방지.

                    **✅ 정리된 예시**

                    **TP 예시 (이익실현):**
                    - 기준: +30%에서 TP 발동, Hysteresis = 2.5%
                    - 한 번 TP가 실행된 후 다시 작동하려면
                    - 👉 수익률이 **27.5% 이하로 떨어져야 함** (30% - 2.5%)
                    - → 즉, 약간의 가격 하락에는 반응하지 않고, 확실히 조정된 뒤에만 재활성화됨.

                    **SL 예시 (손절):**
                    - 기준: –20%에서 SL 발동, Hysteresis = 1.5%
                    - 한 번 SL이 실행된 후 다시 작동하려면
                    - 👉 수익률이 **–18.5% 이상으로 반등해야 함** (–20% + 1.5%)
                    - → 약간의 반등으로 인한 불필요한 손절 반복을 방지.
                    """)
                ...
                with col_h1:
                    ...
                    tp_hysteresis = st.number_input(
                        ...
                        step=0.5,
                        format="%0.1f",
                        help="TP 트리거 후 재활성화를 위해 수익률이 (TP 임계값 - Hysteresis) 이하로 떨어져야 합니다. 예: TP 30%, Hysteresis 2.5% → 27.5% 이하로 떨어져야 재활성화. 기본값: 0 (비활성화). 시장 노이즈로 인한 중복 트리거 방지."
                    )
                with col_h2:
                    ...
                    sl_hysteresis = st.number_input(
                        ...
                        step=0.5,
                        format="%0.1f",
                        help="SL 트리거 후 재활성화를 위해 수익률이 (SL 임계값 + Hysteresis) 이상으로 올라가야 합니다. 예: SL -20%, Hysteresis 1.5% → -18.5% 이상으로 올라가야 재활성화. 기본값: 0 (비활성화). 약간의 반등으로 인한 불필요한 손절 반복 방지."
                    )
